...
public class StatementsFinalizer {
	...
	public static void finalizeStatements(FunctionSection function) {
		UnitSource source = function.sourceRef.source;
		...
		if(function.returnType != VarType.VOID &&
			...
			int sourceLoc = lastStatement == null ? function.sourceRef.stop : lastStatement.sourceRef.stop;
			
			SourceReference returnSourceRef = new SourceReference(source, sourceLoc, -1);
			if(function.returnType instanceof VarCompositeType) {
				...
				statements.add(new CompositeReturnSt(returnSourceRef, returnValues));
			} else {
				Expression returned = StatementParser.getDefaultValue(function.returnType, source, sourceLoc);
				statements.add(new ReturnSt(returnSourceRef, returned));
			}
		}
	...
	private static int closeStatement(List<Statement> statements, int idx) {
		...
		if(toClose.singleLine) {
			if(statements.size() == idx) {
				...
				statements.add(new SectionEndSt(last.sourceRef.collapseToEnd()));
			...
			statements.add(idx, new SectionEndSt(statements.get(idx-1).sourceRef.collapseToEnd()));
			idx++;
